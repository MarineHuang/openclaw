...
export async function feishuSetupPoll(
  ...
  if (raw.client_id && raw.client_secret) {
    return {
      status: "done",
      appId: String(raw.client_id),
      appSecret: String(raw.client_secret),
      openId: raw.user_info
        ? (raw.user_info as Record<string, unknown>).open_id
          ? String((raw.user_info as Record<string, unknown>).open_id)
          : undefined
        : undefined,
    };
  }
